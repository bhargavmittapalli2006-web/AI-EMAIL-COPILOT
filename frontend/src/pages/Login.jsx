...
export const Login = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (authUrlData && authUrlData.authorization_url) {
        window.location.href = authUrlData.authorization_url;
      } else {
        setShowGoogleModal(true);
      }
    } catch (err) {
      console.warn('Google OAuth initiation notice:', err);
      setShowGoogleModal(true);
    } finally {
      setIsGoogleSubmitting(false);
    }
          ...
          <div className="lg:col-span-6 w-full max-w-md mx-auto">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-6 sm:p-8 space-y-5">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2.5 py-2.5 px-4 rounded-xl border border-slate-300 dark:border-slate-700 bg-white hover:bg-slate-50 dark:bg-slate-850 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 font-semibold text-xs transition-colors shadow-2xs cursor-pointer disabled:opacity-60"
              >
                {/* Google G SVG */}
